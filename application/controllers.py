...
def register_routes(app):
    ...
    def register():
        if request.method=='POST':
            ...
            if User.query.filter_by(username=username).first() or User.query.filter_by(email=email).first() or User.query.filter_by(phone_number=phone_number).first():
                flash('User with this username, email or phone number already exists. Please choose some other one.')
                return redirect(url_for('register'))
